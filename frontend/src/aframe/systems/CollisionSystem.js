...
AFRAME.registerSystem('collision', {
  ...
  tick: function(time, deltaTime) {
    ...
    this.asteroids = this.asteroids.filter(a => a.parentNode && !a.components.asteroid.destroyed);
  ...
  checkAsteroidCollisions: function(asteroid) {
    ...
    if (!asteroid.components.asteroid || asteroid.components.asteroid.destroyed) return;

    const asteroidPos = asteroid.object3D.position;
    const asteroidSize = asteroid.components.asteroid.data.size;
  ...
  handleShieldCollision: function(asteroid) {
    ...
    if (asteroid.components.asteroid) {
      asteroid.components.asteroid.destroy(false);
    }
